...
// Stable callback hook (replaces @mui/utils/useEventCallback)
function useEventCallback<T extends (...args: any[]) => any>(fn: T): T {
  const ref = React.useRef<T>(fn);
  React.useInsertionEffect(() => { ref.current = fn; });
  return React.useCallback((...args: Parameters<T>): ReturnType<T> => ref.current(...args), []) as T;
}
import DialogsContext from '@/features/crud/hooks/useDialogs/DialogsContext';
import type { DialogComponent, OpenDialog, OpenDialogOptions } from '@/features/crud/hooks/useDialogs/useDialogs';
